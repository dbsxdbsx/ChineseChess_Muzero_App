...
### 自制基于明确逻辑的引擎思路
0.若当前局面下我方有棋子可把对方的将吃掉，则直接选这个走法；
1.枚举出我方所有“有效”走法；---有效是指走了后局面中对方不存在有棋子可直接吃掉我方将的走法；
...
5.2 排序优先级2的指标：对于5.1排序后局面分数还是一样的情况，则将我方叫将的走法(本回合的第一步棋)设置优先；
5.3 若是5.1和5.2的分数还是一样，则将在本回合对方走后我方没有被将军的局面设置优先；