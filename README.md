...
3.对2.5之后剩下的我方候选走法，将“本回合后”的局面按以下标准按优先级（以下数字越小越优先）排序，选出最高分的走法：
  3.1 排序优先级1的指标：本回合后，设所有我方（除了老将）的棋子在合法走法下走到叫将的最短步数为n，则1/n为该棋子的分数（n为0则得分零），将所有我方的棋子分数累加记为N1，对方到我方的分数为N2。则这个分数为N1-N2。
  3.2 排序优先级2的指标：将本回合我方叫将的走法设置优先；
  3.3 排序优先级3的指标：将本回合后我方没有被将军的局面设置优先；